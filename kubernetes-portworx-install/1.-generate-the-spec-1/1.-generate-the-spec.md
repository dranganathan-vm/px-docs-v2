# 1a. Parameter Reference

This section provides a reference of the parameters available to you when creating a spec file.

Generating a spec file can be done either via the command line or through a GUI wizard. Both methods allow you to use the same parameters, but the parameters are labeled differently. If you are using the command line, you will use the label in the **Label** column as listed below. The GUI wizard uses a corresponding \(and self-explanatory\) label for each one listed below.
